...
namespace {
    ...
    public:
        PreservedAnalyses run(Function &F, FunctionAnalysisManager &FAM) {
            ...
            for (Function::iterator StartBB = F.begin(), E = F.end(); StartBB != E; ++StartBB) {
                ...
                for (BasicBlock::iterator I = StartBB->begin(), IE = StartBB->end(); I != IE; ++I) {
                    ...
                    if (!isa<Instruction>(branch_inst->getOperand(0))) {
                        if (isa<Constant>(branch_inst->getOperand(0))) {
                            ...
                            outs() << "\tBranch condition is not an instruction nor a constant!\n";
                        }
                        continue;
                    }
                    // branch_inst is a conditional branch instruction on an instruction condition.

                    ICmpInst *icmp_inst = dyn_cast<ICmpInst>(&*(branch_inst->getOperand(0)));
                    ...
                    // branch_inst is a conditional branch instruction on an icmp instruction condition, icmp_inst.

                    // If branch_inst's branch condition is not an `icmp eq` instruction, ignore.
                    if (icmp_inst->getPredicate() != ICmpInst::Predicate::ICMP_EQ) {
                        outs() << "\tBranch condition register is not a icmp eq instruction!\n";
                        continue;
                    }
                    // branch_inst is a conditional branch instruction on an `icmp eq` instruction condition, icmp_inst.
                    ...
                    // branch_inst is a conditional branch instruction on an `icmp eq` instruction condition, icmp_inst,
                    // which in turn has two non-constant operands.
